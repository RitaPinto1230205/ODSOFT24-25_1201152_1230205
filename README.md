...
Este documento detalha as classes de testes e os casos de testes associados ao sistema, abrangendo testes de integração e unidade.

| Test Class                          | Test Name                                             | Propósito                                         | Descrição                                                                                         |
|-------------------------------------|------------------------------------------------------|---------------------------------------------------|-----------------------------------------------------------------------------------------------------|
| `AuthorRepositoryIntegrationTest`   | `whenFindByName_thenReturnAuthor()`                  | Verifica que um autor é retornado ao ser pesquisado pelo nome usando o `AuthorRepository`. | Confirma que o objeto autor correto é retornado do banco de dados quando pesquisado pelo nome.       |
| `AuthorControllerIntegrationTest`   | (Vazio)                                              | Nenhum teste definido, a classe inclui apenas `@BeforeEach` setup. | Contém configuração para testes de integração, mas não inclui casos de teste reais.                   |
| `AuthorServiceImplIntegrationTest`  | `whenValidId_thenAuthorShouldBeFound()`              | Verifica que um autor é encontrado quando um ID válido é fornecido usando o `AuthorService`. | Verifica se o serviço recupera corretamente o autor com o ID especificado.                       |
| `BookControllerIntegrationTest`     | `createBook_ShouldReturnCreatedStatus()`             | Verifica que um livro é criado e retorna um status de `Criado` com o título correto. | Garante que um livro é criado no sistema e que a API retorna um status 201 Created.            |
|                                     | `findByIsbn_ShouldReturnBook()`                      | Verifica que um livro é encontrado pelo ISBN e retorna informações corretas. | Verifica se os detalhes corretos do livro são retornados quando pesquisados pelo ISBN.                       |
|                                     | `deleteBookPhoto_ShouldReturnOkStatus()`             | Verifica que uma foto de livro é excluída e retorna um status de `200 OK`. | Garante que a exclusão da foto de um livro é bem-sucedida e a API retorna um status 200.      |
|                                     | `getSpecificBookPhoto_ShouldReturnPhoto()`           | Verifica que a foto de um livro é retornada corretamente. | Confirma que a foto correta para um livro específico é retornada pela API.                     |
|                                     | `updateBook_ShouldReturnUpdatedBook()`               | Verifica que a atualização de um livro funciona e retorna o livro atualizado. | Garante que as alterações feitas em um livro são refletidas nos dados retornados após a atualização.          |
|                                     | `findBooks_ShouldReturnListOfBooks()`                | Verifica que uma lista de livros é retornada ao pesquisar pelo título. | Confirma que a API retorna a lista correta de livros que correspondem aos critérios de pesquisa.           |
|                                     | `getTop5BooksLent_ShouldReturnTop5Books()`          | Verifica que os 5 livros mais emprestados são retornados corretamente. | Verifica se a API retorna os 5 livros mais emprestados com base nos dados de empréstimo.                        |
| `BookRepositoryIntegrationTest`     | `testFindByGenre()`                                 | Verifica que os livros são retornados corretamente ao serem pesquisados por gênero. | Confirma que a lista esperada de livros é retornada para uma consulta de gênero específica.                  |
|                                     | `testFindByTitle()`                                 | Verifica que os livros são retornados corretamente ao serem pesquisados por título. | Garante que os livros corretos são recuperados ao serem pesquisados por título.                              |
|                                     | `testFindByAuthorName()`                            | Verifica que os livros são retornados corretamente ao serem pesquisados pelo nome do autor. | Verifica se a lista correta de livros é retornada para um autor específico.                          |
|                                     | `testFindByIsbn()`                                 | Verifica que um livro é encontrado pelo ISBN. | Confirma que o livro correto é recuperado ao ser pesquisado pelo ISBN.                             |
|                                     | `testFindBooksByAuthorNumber()`                     | Verifica que os livros são retornados corretamente ao serem pesquisados pelo ID do autor. | Garante que os livros esperados são retornados para o ID do autor fornecido.                            |
...
|                                     | `testGetLendingsAverageDurationPerMonthInvalidDates()` | Verifica que um erro é gerado para intervalos de datas inválidas ao calcular a duração do empréstimo. | Confirma que um erro apropriado é gerado para parâmetros de data inválidos ao calcular durações. |
...
![Jenkins_DEI.png](Docs/images/PipelineJenkins.png)
